...
class OAuth2AuthenticationSuccessHandlerTest {
    ...
    class RedirectUrlTest {
        ...
        void pendingUser_redirectsToOnboarding() throws Exception {
            ...
            String origin = "LOCAL_AUDIENCE_URL";
            ...
            assertThat(redirectUrl).isEqualTo("LOCAL_AUDIENCE_URL/onboarding");
        ...
        void completedUser_domainMatch_redirectsToMain() throws Exception {
            ...
            // Then
            String redirectUrl = response.getRedirectedUrl();
            assertThat(redirectUrl).isEqualTo("LOCAL_AUDIENCE_URL");
            assertThat(redirectUrl).doesNotContain("token=");
        ...
        void completedUser_domainMismatch_redirectsToCorrectDomain() throws Exception {
            ...
            String wrongOrigin = "LOCAL_AUDIENCE_URL"; // AUDIENCE 도메인
            String correctDomain = "LOCAL_ORGANIZER_URL"; // ORGANIZER 도메인
            ...
            // Then
            String redirectUrl = response.getRedirectedUrl();
            assertThat(redirectUrl).isEqualTo("LOCAL_ORGANIZER_URL");
        }
        ...
        void production_pendingUser_redirectsToOnboarding() throws Exception {
            // Given
            String email = "new@example.com";
            String origin = "PROD_AUDIENCE_URL";
            User user = createUser(email, RegistrationStatus.PENDING, null);
            ...
            // Then
            String redirectUrl = response.getRedirectedUrl();
            assertThat(redirectUrl).isEqualTo("PROD_AUDIENCE_URL/onboarding");
        }
        ...
        void production_completedUser_redirectsToMain() throws Exception {
            // Given
            String email = "existing@example.com";
            String origin = "PROD_ORGANIZER_URL";
            User user = createUser(email, RegistrationStatus.COMPLETED, UserType.ORGANIZER);
            ...
            // Then
            String redirectUrl = response.getRedirectedUrl();
            assertThat(redirectUrl).isEqualTo("PROD_ORGANIZER_URL");
        }
    }
    ...
    class CookieTest {
        ...
        @Test
        @DisplayName("프로덕션 환경 - SameSite=None, Secure=true, Domain=PROD_COOKIE_DOMAIN")
        void productionEnvironment_cookieSettings() throws Exception {
            ...
            when(domainRoleMapping.getCookieDomain(origin)).thenReturn("PROD_COOKIE_DOMAIN");
            ...
            assertThat(setCookie).contains("Domain=PROD_COOKIE_DOMAIN");
            assertThat(setCookie).contains("HttpOnly");
        }
    ...
    class UserTypeSaveTest {
        ...
        void pendingUser_userTypeSaved() throws Exception {
            // Given
            String email = "new@example.com";
            String origin = "LOCAL_ORGANIZER_URL";
            User user = createUser(email, RegistrationStatus.PENDING, null);
